...
def THDatatable():
    ...
    count_list = list((number1, number2, number3, number4))
    client.close()
    return render_template('THDatatable.html', count=count_list)
...
def THDatatableAct():
    if request.method == 'POST':
        ...
        data_one = list(collection.find({"time": {"$regex": query}}))
        print(data_one)
        collection = db.relic
        number3 = collection.estimated_document_count()
        collection = db.sensor
        ...
        count_list = list((number1, number2, number3, number4))
        client.close()
        return render_template('THDatatableAct.html', count=count_list)
...
def riskDatatable():
    ...
    count_list = list((number1, number2, number3, number4))
    client.close()
    return render_template('riskDatatable.html', count=count_list)
