...
def index():
    ...
    collection = db.thdata
    ex_date = list(collection.find({}, {"year": {"$substr": ["$time", 0, 4]}, "_id": 0}))
    print(ex_date)
    client.close()
...
def visualization():
    ...
    return render_template('visualization.html', count=count_list, z_result=z_result, th_result=th_tmp,
                           fig_code=fig_code,
                           slide_num=slide_num, total_num=total_num)
...
def THDatatable():
    ...
    collection = db.thdata
    th_tmp = list()
    slide_num = collection.estimated_document_count()
    total_num = int(slide_num / number2)
    print(total_num)
    slide_num = slide_num / number2
    slide_num = math.ceil(slide_num / 10)
...
def THDatatableAct():
    if request.method == 'POST':
        ...
        return render_template('THDatatableAct.html', count=count_list, z_result=z_result, th_result=th_tmp,
                               day_re=result, slide_num=slide_num, total_num=total_num, fig_code=fig_code)
...
def tlqkfwha():
    ...
    th_result = list(collection.find())
    th_frame = pd.DataFrame(th_result)

    # 판다 데이터프레임에서 칼럼만 추출
    random_x = th_frame['date']
    random_y0 = th_frame['tem']
    random_y1 = th_frame['hu']
    ...
    return render_template('tlqkfwha.html', z_result=z_result, js_tmp=fig.to_json())
