...
input_var = ['tm', 'ta', 'pv', 'hm','td', 'm005Te', 'm01Te', 'm02Te', 'm03Te']
...
#th_frame=th_frame.drop(['humidity'], axis=1)
...
result_df.to_csv('weather(2열).csv')
print(result_df)
...
print(training_set[:, 1:dataset_train.shape[1]])
...
for i in range(n_past, len(training_set_scaled) - n_future + 1):
    ...
    y_train.append(training_set_scaled[i + n_future - 1:i + n_future, 0:1])
...
test_y = pd.DataFrame(y_test_inv, columns=['tem'])
pred_y = pd.DataFrame(y_pred_inv, columns=['tem'])
