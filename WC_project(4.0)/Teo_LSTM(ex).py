...
for i in range(n_past, len(training_set_scaled) - n_future + 1):
    ...
    y.append(training_set_scaled[i:i + n_future, 0])
    yz.append(training_set[i:i + n_future, 0])

sc_predict = MinMaxScaler()
sc_predict.fit_transform(yz)  # output만 정규화
...
plt.plot(y_test_inv[0:1].T, label="Actual")
plt.plot(y_pred_inv[0:1].T, label="Predict")
