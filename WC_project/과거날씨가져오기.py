...
min_time = list(
    collection.find({"zone_name": "대웅전-내부 좌측-4번-직지사"}).sort([("time", 1)]).limit(1))  # 데이터가 어디부터 어디까지 들어있는지 볼때
max_time = list(collection.find({"zone_name": "대웅전-내부 좌측-4번-직지사"}).sort([("time", -1)]).limit(1))

min_date = min_time[0]["time"][0:10]
max_date = max_time[0]["time"][0:10]
diff = datetime.strptime(max_time[0]["time"][0:10], "%Y-%m-%d") - datetime.strptime(min_time[0]["time"][0:10],
                                                                                    "%Y-%m-%d")
print(diff.days)
num = diff.days + 1
...
dataCd = 'ASOS'  # 자료코드..?
dateCd = 'HR'  # 날짜코드?
stnIds = '135'  # 지점번호 어느지역 기상청인지 강진 259 여수 168 추풍령(김천) 135
endDt = max_date  # 종료날짜
endHh = '23'  # 종료시간
startHh = '00'
startDt = min_date
...
result_df['rn'] = result_df['rn'].fillna(0)  # 결측값 채우기
result_df = result_df.apply(pd.to_numeric)  # 숫자로 변환
# corr_data = result_df.corr(method='pearson')
# corr_data.to_csv('corr_dataxx.csv')
result_df = result_df.dropna(axis=0)  # 결측값 존재시 제거
result_df = result_df.reset_index()  # 인덱스 리셋
result_df.to_csv('weather_직지사.csv')  # 저장
